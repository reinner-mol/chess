var LichessRacer = (function (exports) {
    'use strict';
    ...
    function parseUci(str) {
        if (str[1] === '@' && str.length === 4) {
            ...
            if (defined$1(from) && defined$1(to))
    ...
    var Boost = /** @class */ (function () {
        function Boost() {
            ...
            this.setPlayers = function (players) {
                if (players.length != _this.cars.length) {
                    _this.cars = players.map(function (p) { return ({ score: p.score, time: -9999999 }); });
                }
                else {
                    _this.cars = _this.cars.map(function (car, i) { return ({
                        score: players[i].score,
                        time: players[i].score > car.score ? getNow() : car.time
                    }); });
                }
            };
            this.isBoosting = function (index) { var _a, _b; return ((_a = _this.cars[index]) === null || _a === void 0 ? void 0 : _a.score) && ((_b = _this.cars[index]) === null || _b === void 0 ? void 0 : _b.time) > getNow() - 1000; };
        }
        return Boost;
    }());
    ...
    var Countdown = /** @class */ (function () {
        function Countdown(clock, resetGround, redraw) {
            var _this = this;
            this.clock = clock;
            this.resetGround = resetGround;
            this.redraw = redraw;
            this.played = new Set();
            this.start = function (startsAt, aloud) {
                var countdown = function () {
                    var diff = startsAt.getTime() - Date.now();
                    if (diff > 0) {
                        if (aloud && diff % 1000 > 500)
                            _this.playOnce(Math.ceil(diff / 1000));
            ...
            this.playOnce = function (i) {
                if (!_this.played.has(i)) {
                    _this.played.add(i);
                    lichess.sound.play("countDown" + i);
                }
            };
            for (var i = 10; i >= 0; i--)
                lichess.sound.loadStandard("countDown" + i);
        }
        return Countdown;
    }());
    ...
    var StormCtrl = /** @class */ (function () {
        function StormCtrl(opts, redraw) {
            var _this = this;
            this.sign = Math.random().toString(36);
            this.localScore = 0;
            this.boost = new Boost();
            this.skipAvailable = true;
            this.knowsSkip = storedProp('racer.skip', false);
            this.ground = prop(false);
            this.flipped = false;
            this.serverUpdate = function (data) {
                _this.data.players = data.players;
                _this.boost.setPlayers(data.players);
                if (data.startsIn) {
                    _this.vm.startsAt = new Date(Date.now() + data.startsIn);
                    if (data.startsIn > 0)
                        _this.countdown.start(_this.vm.startsAt, _this.isPlayer());
                    else
                        _this.run.clock.start();
                }
            };
            this.player = function () { return _this.data.player; };
            this.players = function () { return _this.data.players; };
            this.isPlayer = function () { return !_this.vm.alreadyStarted && _this.data.players.some(function (p) { return p.name == _this.data.player.name; }); };
            this.raceFull = function () { return _this.data.players.length >= 10; };
            this.status = function () { return (_this.run.clock.started() ? (_this.run.clock.flag() ? 'post' : 'racing') : 'pre'); };
            this.isRacing = function () { return _this.status() == 'racing'; };
            this.myScore = function () {
                var p = _this.data.players.find(function (p) { return p.name == _this.data.player.name; });
                return p === null || p === void 0 ? void 0 : p.score;
            };
            this.join = throttle(1000, function () {
                if (!_this.isPlayer())
                    _this.socketSend('racerJoin');
            });
            this.countdownSeconds = function () {
                return _this.status() == 'pre' && _this.vm.startsAt && _this.vm.startsAt > new Date()
                    ? Math.min(9, Math.ceil((_this.vm.startsAt.getTime() - Date.now()) / 1000))
                    : undefined;
            };
            this.end = function () {
                _this.resetGround();
            ...
            this.canSkip = function () { return _this.skipAvailable; };
            this.skip = function () {
                if (_this.skipAvailable && _this.run.clock.started()) {
                    _this.skipAvailable = false;
                    sound.good();
                    _this.playUci(_this.run.current.expectedMove());
                    _this.knowsSkip(true);
                }
            };
            this.userMove = function (orig, dest) {
                if (!_this.promotion.start(orig, dest, _this.playUserMove))
                    _this.playUserMove(orig, dest);
            };
            this.playUserMove = function (orig, dest, promotion) {
                return _this.playUci("" + orig + dest + (promotion ? (promotion == 'knight' ? 'n' : promotion[0]) : ''));
            };
            this.playUci = function (uci) {
                var now = getNow();
                var puzzle = _this.run.current;
                if (puzzle.startAt + config.minFirstMoveTime > now)
                    console.log('reverted!');
                else {
                    _this.run.moves++;
                    _this.promotion.cancel();
                    var pos = puzzle.position();
                    var move = parseUci(uci);
                    var captureSound = pos.board.occupied.has(move.to);
                    pos.play(move);
                    if (pos.isCheckmate() || uci == puzzle.expectedMove()) {
                        puzzle.moveIndex++;
                        _this.localScore++;
                        _this.run.combo.inc();
                        _this.run.modifier.moveAt = now;
                        var bonus = _this.run.combo.bonus();
                        if (bonus) {
                            _this.run.modifier.bonus = bonus;
                            _this.localScore += bonus.seconds; // yeah, ah well
                        }
                        _this.socketSend('racerScore', _this.localScore);
                        if (puzzle.isOver()) {
                            if (!_this.incPuzzle())
                                _this.end();
                        }
                        ...
                        sound.wrong();
                        _this.run.errors++;
                        _this.run.combo.reset();
                        if (_this.run.clock.flag())
                            _this.end();
                        else if (!_this.incPuzzle())
                            _this.end();
                    }
                    _this.redraw();
                    _this.redrawQuick();
                ...
                _this.resetGround();
                lichess.pubsub.emit('ply', _this.run.moves);
            };
            this.redrawQuick = function () { return setTimeout(_this.redraw, 100); };
            this.redrawSlow = function () { return setTimeout(_this.redraw, 1000); };
            this.cgOpts = function () {
                return _this.isPlayer()
                    ? makeCgOpts(_this.run, _this.isRacing(), _this.flipped)
                    : {
                        orientation: _this.run.pov
                    };
            };
            this.resetGround = function () { return _this.withGround(function (g) { return g.set(_this.cgOpts()); }); };
            this.incPuzzle = function () {
                var index = _this.run.current.index;
                if (index < _this.data.puzzles.length - 1) {
                    _this.run.current = new CurrentPuzzle(index + 1, _this.data.puzzles[index + 1]);
                    return true;
                }
                return false;
            };
            this.withGround = function (f) {
                var g = _this.ground();
                return g && f(g);
            };
            this.flip = function () {
                _this.flipped = !_this.flipped;
                _this.withGround(function (g) { return g.toggleOrientation(); });
                _this.redraw();
            };
            this.socketSend = function (tpe, data) { return lichess.socket.send(tpe, data, { sign: _this.sign }); };
            this.hotkeys = function () { return window.Mousetrap.bind('f', _this.flip); };
            this.data = opts.data;
            this.race = this.data.race;
            this.pref = opts.pref;
            this.redraw = function () { return redraw(_this.data); };
            ...
            this.countdown = new Countdown(this.run.clock, this.resetGround, function () { return setTimeout(_this.redraw); });
            ...
            lichess.socket = new lichess.StrongSocket("/racer/" + this.race.id, false, {
                events: {
                    racerState: function (data) {
                        _this.serverUpdate(data);
                        _this.redraw();
                        _this.redrawSlow();
                    }
                }
    ...
    ***************************************************************************** */
    ...
    var trackHeight = 25;
    var renderRace = function (ctrl) {
        var players = ctrl.players();
        var minMoves = players.reduce(function (m, p) { return (p.score < m ? p.score : m); }, 130) / 3;
        var maxMoves = players.reduce(function (m, p) { return (p.score > m ? p.score : m); }, 35);
        var delta = maxMoves - minMoves;
        var relative = function (score) { return (score - minMoves) / delta; };
        var bestScore = players.reduce(function (b, p) { return (p.score > b ? p.score : b); }, 0);
        var myName = ctrl.player().name;
        var tracks = [];
        players.forEach(function (p, i) {
            var isMe = p.name == myName;
            var track = renderTrack(relative, isMe, bestScore, ctrl.boost, p, i);
            if (isMe)
                tracks.unshift(track);
            else
                tracks.push(track);
        });
        return h('div.racer__race', {
            attrs: {
                style: "height:" + (players.length * trackHeight + 14) + "px"
            }
        }, h('div.racer__race__tracks', tracks));
    };
    var renderTrack = function (relative, isMe, bestScore, boost, player, index) {
    ...
    var playerLink = function (player, isMe) {
    ...
    var renderGround = function (ctrl) {
        return h('div.cg-wrap', {
            hook: {
                insert: function (vnode) {
                    return ctrl.ground(Chessground(vnode.elm, makeConfig(ctrl.isRacing() && ctrl.isPlayer()
                        ? makeCgOpts(ctrl.run, true)
                        : {
                            fen: INITIAL_BOARD_FEN,
                            orientation: ctrl.run.pov,
                            movable: { color: ctrl.run.pov }
                        }, ctrl.pref, ctrl.userMove)));
                }
            }
        });
    };
    var renderCountdown = function (seconds) {
    ...
    function view (ctrl) {
        var _a;
        return h('div.racer.racer-app.racer--play', {
            "class": __assign(__assign({}, playModifiers(ctrl.run)), (_a = {}, _a["racer--" + ctrl.status()] = true, _a))
        }, [renderRace(ctrl), renderBoard(ctrl), h('div.puz-side', selectScreen(ctrl))]);
    }
    var selectScreen = function (ctrl) {
        var noarg = ctrl.trans.noarg;
        switch (ctrl.status()) {
            case 'pre': {
                var povMsg = h('p.racer__pre__message__pov', ctrl.trans(povMessage(ctrl.run)));
            ...
            case 'racing': {
                var clock = renderClock(ctrl.run, ctrl.end, false);
            ...
            case 'post': {
                var nextRace = ctrl.race.lobby ? lobbyNext(ctrl) : friendNext(ctrl);
                var raceComplete = h('h2', noarg('raceComplete'));
                return ctrl.isPlayer()
    ...
    var renderSkip = function (ctrl) {
        return h('button.racer__skip.button.button-red', {
            "class": {
    ...
    var skipHelp = function (noarg) { return h('p', noarg('skipHelp')); };
    var puzzleRacer = function () { return h('strong', 'Puzzle Racer'); };
    var waitingToStart = function (noarg) {
        return h('div.puz-side__top.puz-side__start', h('div.puz-side__start__text', [puzzleRacer(), h('span', noarg('waitingToStart'))]));
    };
    var spectating = function (noarg) {
        return h('div.puz-side__top.puz-side__start', h('div.puz-side__start__text', [puzzleRacer(), h('span', noarg('spectating'))]));
    };
    var renderBonus = function (bonus) { return "+" + bonus; };
    var renderControls = function (ctrl) {
        return h('div.puz-side__control', h('a.puz-side__control__flip.button', {
            "class": {
                active: ctrl.flipped,
    ...
    var comboZone = function (ctrl) {
        return h('div.puz-side__table', [renderControls(ctrl), renderCombo(config, renderBonus)(ctrl.run)]);
    };
    var playerScore = function (ctrl) {
        return h('div.puz-side__top.puz-side__solved', [h('div.puz-side__solved__text', ctrl.myScore() || 0)]);
    };
    var renderLink = function (ctrl) {
        return h('div.puz-side__link', [
    ...
    var renderJoin = function (ctrl) {
        return h('div.puz-side__join', h('button.button.button-fat', {
            hook: bind('click', ctrl.join)
        }, ctrl.trans.noarg('joinTheRace')));
    };
    var yourRank = function (ctrl) {
        var score = ctrl.myScore();
        if (!score)
            return;
        var players = ctrl.players();
        var rank = players.filter(function (p) { return p.score > score; }).length + 1;
        return h('strong.race__post__rank', ctrl.trans('yourRankX', rank + "/" + players.length));
    };
    var waitForRematch = function (noarg) {
        return h("a.racer__new-race.button.button-fat.button-navaway.disabled", {
            attrs: { disabled: true }
        }, noarg('waitForRematch'));
    };
    var lobbyNext = function (ctrl) {
        return h('form', {
            attrs: {
                action: '/racer/lobby',
                method: 'post'
            ...
            h("button.racer__new-race.button.button-navaway" + (ctrl.race.lobby ? '.button-fat' : '.button-empty'), ctrl.trans.noarg('nextRace')),
        ]);
    };
    var friendNext = function (ctrl) {
        return h('div.racer__post__next', [
            h("a.racer__rematch.button.button-fat.button-navaway", {
                attrs: { href: "/racer/" + ctrl.race.id + "/rematch" }
            }, ctrl.trans.noarg('joinRematch')),
            h('form.racer__post__next__new', {
                attrs: {
                    action: '/racer',
                    method: 'post'
                }
            }, h('button.racer__post__next__button.button.button-empty', {
                attrs: {
                    type: 'submit'
                }
    ...
    function start(opts) {
        var element = document.querySelector('.racer-app');
        var vnode;
        function redraw() {
            vnode = patch(vnode, view(ctrl));
        }
        var ctrl = new StormCtrl(opts, redraw);
        var blueprint = view(ctrl);
        element.innerHTML = '';
