(function () {
	...
	const setupActionForm = () => {
	    ...
	    const debouncedSubmit = debounce(() => formToXhr(form).then(() => {
	    ...
	    $(form).on('click', 'button', e => {
	        ...
	        if (nbSelected < 1)
	            return false;
	        if (action == 'analyse') {
	            if (nbSelected >= 20 && !confirm(`Analyse ${nbSelected} games?`))
	                return;
	...
	const shiftClickCheckboxRange = (table) => {
	    let lastChecked;
	    const checkIntermediateBoxes = (first, last) => {
	        ...
	        for (const input of table.querySelectorAll('tbody input')) {
	            if (first == input || last == input) {
	                ...
	                input.checked = last.checked;
	        }
	    };
	    return (input, shift) => {
	        if (shift && lastChecked && input != lastChecked)
	            checkIntermediateBoxes(lastChecked, input);
	        lastChecked = input;
	    };
	};
	const expandCheckboxZone = (table, onSelect) => $(table).on('click', 'td:first-child', (e) => {
	    if (e.target.tagName == 'INPUT')
	        onSelect(e.target, e.shiftKey);
	    else {
