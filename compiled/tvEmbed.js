(function () {
    ...
    function resize() {
        ...
        if (el.offsetHeight > window.innerHeight)
            el.style.maxWidth =
                window.innerHeight - el.querySelector('.mini-game__player').offsetHeight * 2 + 'px';
    }
    window.onload = () => {
        loadClockWidget();
        const findGame = () => document.getElementsByClassName('mini-game').item(0);
        const setup = () => init(findGame());
        setup();
        if (window.EventSource)
            new EventSource(document.body.getAttribute('data-stream-url')).addEventListener('message', e => {
